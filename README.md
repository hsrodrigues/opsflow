...
| **Ocorrências** | CRUD completo (seção 14) — tipo configurável, severidade, status, vínculo com veículo/motorista; registrar um "Acidente" bloqueia o veículo automaticamente, e severidade Crítica bloqueia o motorista |
...
| **Notificações & automações** | 5 robôs em background (APScheduler): detecção automática de atraso, alerta de CNH vencendo, expiração automática de licença, backup diário do banco, fechamento de operações penduradas; sino de notificações no desktop mostra tudo isso em tempo real |
...
144 testes automatizados (93% de cobertura de linha), sempre contra um
...
exportação, o bloqueio automático de veículo ao registrar um acidente e de
motorista ao registrar uma ocorrência crítica, o console de plataforma
(onboarding de empresa, troca de plano, suspensão — sempre confirmando que
nenhum papel de empresa cliente chega perto disso), o fluxo de ativação por
chave solta (gerar → resgatar sem autenticação → login automático → reuso
bloqueado), os endpoints de backup/restore (permissão restrita a
`SUPER_ADMIN`; a chamada real a `mysqldump`/`mysql` é mockada aqui —
verificada à parte, contra um MySQL de verdade) e o hardening de segurança
(rate limiting, docs desligados fora de dev, `/api/health` sem vazar
credencial). Os 5 robôs em background são testados chamando `run()`
diretamente, nunca via APScheduler de verdade (evitaria uma suíte
instável).
...
| 9 | Testes (cobertura ampla, isolamento multi-tenant) | ✅ Concluída — 144 testes, 93% de cobertura de linha |
