"""Products screen: pesquisa, filtros, paginação, edição, exclusão."""
...
_STATUS_FILTER_OPTIONS = [("Todos os status", None), ("Ativo", "ATIVO"), ("Inativo", "INATIVO")]
_STATUS_DISPLAY = {"ATIVO": ("Ativo", "BadgeSuccess"), "INATIVO": ("Inativo", "BadgeNeutral")}
_UNIT_LABELS = {
    "UNIDADE": "Unidade", "KG": "kg", "TONELADA": "t", "LITRO": "L",
    "CAIXA": "Caixa", "PALETE": "Palete", "METRO_CUBICO": "m³",
}
_PAGE_SIZE = 15
...
class ProductsPage(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        header = QHBoxLayout()
        header.addWidget(QLabel("Produtos", objectName="PageTitle"))
        header.addStretch(1)
        new_button = QPushButton("+ Novo Produto", objectName="PrimaryButton")
        ...
        self._search_input.setPlaceholderText("Buscar por nome ou SKU...")
        ...
        self._table.setHorizontalHeaderLabels(["Nome", "SKU", "Unidade", "Status", ""])
    ...
    def _load_products(self) -> None:
        ...
        self._worker = ApiWorker(
            self._api_client.list_products, self._session.access_token,
    ...
    def _handle_products_loaded(self, result: dict) -> None:
        ...
        for row, product in enumerate(items):
            ...
            self._table.setItem(row, 1, QTableWidgetItem(product.get("sku") or "—"))
            unit_label = _UNIT_LABELS.get(product["unit_of_measure"], product["unit_of_measure"])
            self._table.setItem(row, 2, QTableWidgetItem(unit_label))
            text, badge_class = _STATUS_DISPLAY.get(product["status"], (product["status"], "BadgeNeutral"))
            self._table.setCellWidget(row, 3, build_badge(text, badge_class))
            self._table.setCellWidget(row, 4, self._build_row_actions(product))

        self._page_label.setText(f"Página {meta['page']} de {meta['total_pages']} — {meta['total']} produto(s)")
    ...
    def _handle_load_failed(self, exc: Exception) -> None:
        message = exc.friendly_message if isinstance(exc, ApiError) else "Não foi possível carregar os produtos."
        self._show_status(message, is_error=True)

    def _build_row_actions(self, product: dict) -> QWidget:
        container = QWidget()
        row_layout = QHBoxLayout(container)
        row_layout.setContentsMargins(0, 0, 0, 0)
        edit_button = QPushButton("Editar", objectName="LinkButton")
        edit_button.setCursor(Qt.CursorShape.PointingHandCursor)
        edit_button.clicked.connect(lambda: self._handle_edit_clicked(product))
        delete_button = QPushButton("Excluir", objectName="DangerLinkButton")
        delete_button.setCursor(Qt.CursorShape.PointingHandCursor)
        delete_button.clicked.connect(lambda: self._handle_delete_clicked(product))
    ...
    def _handle_new_clicked(self) -> None:
        dialog = ProductDialog(self._api_client, self._session.access_token)
        if dialog.exec():
            self._show_status(f"Produto {dialog.saved_product['name']} criado com sucesso.")
            self._load_products()

    def _handle_edit_clicked(self, product: dict) -> None:
        dialog = ProductDialog(self._api_client, self._session.access_token, product=product)
        if dialog.exec():
            self._show_status(f"Produto {dialog.saved_product['name']} atualizado com sucesso.")
            self._load_products()

    def _handle_delete_clicked(self, product: dict) -> None:
        ...
        if confirmation != QMessageBox.StandardButton.Yes:
            return
        worker = ApiWorker(self._api_client.delete_product, self._session.access_token, product["id"])
        worker.succeeded.connect(lambda _r: self._handle_delete_succeeded(product["name"]))
        worker.failed.connect(self._handle_load_failed)
        worker.start()
        self._delete_worker = worker

    def _handle_delete_succeeded(self, name: str) -> None:
        self._show_status(f"Produto {name} excluído.")
        self._load_products()
